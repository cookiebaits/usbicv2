...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!user.pendingCryptoTransfer) return NextResponse.json({ error: "No pending crypto transfer found" }, { status: 400 });

        const timeElapsed = Date.now() - new Date(user.pendingCryptoTransfer.createdAt).getTime();
        ...
        const transaction = Transaction.create({
            userId: user.id, description: `Bitcoin Sent to ${recipientWallet}`, amount: 0,
            type: "bitcoin_transfer", category: "Crypto Transfer", accountType: "crypto",
            status: "completed", cryptoAmount: -amount, cryptoPrice: btcPrice,
            recipientWallet, memo: memo || "",
        }) as any;
        ...
        return NextResponse.json({
            ...
            newCryptoBalance: user.cryptoBalance, transactionId: transaction.id, amount, recipientWallet, memo,
        });
    } catch (error) {
