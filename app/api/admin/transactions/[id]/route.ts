    ...
    for (const originalTransaction of transactions) {
      ...
      if (isTransfer) {
        ...
        if (originalTransaction.type === "crypto_buy") {
          userUpdate.balanceChange += -originalTransaction.amount;
          userUpdate.cryptoBalanceChange += -(originalTransaction.cryptoAmount || 0);
  ...
  try {
    ...
    if (isCrypto) {
      if (type === "crypto_buy") {
        ...
        cryptoBalanceChange = cryptoAmountDifference;
      ...
      if (transaction.accountType === "checking") {
        balanceChange = amountDifference;
        savingsBalanceChange = -amountDifference;
    ...
    if (transaction.type === "transfer" && transaction.category === "Transfer" && transaction.transferId) {
      const other = await Transactionnn.findOne({
        transferId: transaction.transferId,
        _id: { $ne: transaction._id },
      });
      if (other) {
        other.amount = -transaction.amount;
        other.description = transaction.description;
        other.memo = transaction.memo;
        other.date = transaction.date;
        await other.save();
        console.log("DEBUG: Updated paired transfer transaction:", other._id);
      }
    }
